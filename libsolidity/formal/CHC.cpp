...
void CHC::endVisit(FunctionDefinition const& _function)
{
	...
	if (!_function.isConstructor())
	{
		...
		auto const& resolved = contractFunctions(*m_currentContract);
		if (
			find(begin(resolved), end(resolved), &_function) != resolved.end() &&
			_function.isPublic()
		)
		{
...
void CHC::defineInterfacesAndSummaries(SourceUnit const& _source)
{
	for (auto const& node: _source.nodes())
		if (auto const* contract = dynamic_cast<ContractDefinition const*>(node.get()))
		{
			...
			set<FunctionDefinition const*> resolvedSet(begin(resolved), end(resolved));
			auto toVisit = resolvedSet;

			for (auto const* base: contract->annotation().linearizedBaseContracts)
				for (auto const* baseFun: base->definedFunctions())
					if (!baseFun->isConstructor() && baseFun->virtualSemantics())
						toVisit.insert(baseFun);

			for (auto const* function: toVisit)
			{
				...
				if (!function->isConstructor() && function->isPublic() && resolvedSet.count(function))
				{
